...
def render_opportunity_memo_markdown(
    *,
    ...
    lines.append(_clean(memo.why_this_is_an_opportunity))
    lines.append("")
    lines.append("## Trade Expression")
    lines.append(_clean(memo.trade_expression))
    lines.append("")
    lines.append("## Quantified Evidence Points")
    for row in memo.quantified_evidence_points:
        lines.append(f"- {_clean(row)}")
    lines.append("")
    lines.append("## Risks")
    ...
    for row in memo.invalidation_triggers:
        lines.append(f"- {_clean(row)}")
    lines.append("")
    ...
    lines.append(_clean(memo.confidence_level))
    lines.append("")
...
def render_opportunity_memo_telegram_html(
    *,
    ...
    _section("Core Thesis", memo.core_thesis_one_liner)
    _section("Opportunity Target", memo.opportunity_target)
    _section("Market Setup", memo.market_setup)
    _section("Primary Driver", memo.primary_driver)
    _section_list("Quantified Evidence", memo.quantified_evidence_points)
    _section("Why Now", memo.why_now)
    _section("Why This Is An Opportunity", memo.why_this_is_an_opportunity)
    _section("Trade Expression", memo.trade_expression)
    ...
    _section_list("Invalidation Triggers", memo.invalidation_triggers)
    _section_list("Watchpoints", memo.watchpoints)
    _section("Confidence Level", memo.confidence_level)
